...
void test_insert()
{
  ...
  for (int i = 0; i < insert_num; i++) {
    ...
    if (i % TIMES == 1) {
      ...
      rc = handler->insert_entry((const char *)&i, &rid, false);
...
void test_delete()
{
  ...
  for (int i = 0; i < insert_num / 2; i++) {
    ...
    if (t == 0 || t == 1) {
      ...
      rc = handler->delete_entry((const char *)&i, &rid, false);
      if (rc != RC::SUCCESS) {
...
TEST(test_bplus_tree, test_leaf_index_node_handle)
{
  ...
  index_file_header.attr_length.push_back(4);
  index_file_header.key_length = 4 + sizeof(RID);
  index_file_header.attr_type.push_back(INTS);
...
TEST(test_bplus_tree, test_chars)
{
  ...
  std::vector<AttrType> types;
  types.push_back(CHARS);
  std::vector<int> lens;
  lens.push_back(8);
  handler->create(index_name, types, lens, false, ORDER, ORDER);
  ...
  for (size_t i = 0; i < sizeof(keys)/sizeof(keys[0]); i++) {
    rid.page_num = 0;
    rid.slot_num = i;
    rc = handler->insert_entry(keys[i], &rid, false);
    ASSERT_EQ(RC::SUCCESS, rc);
  }
...
TEST(test_bplus_tree, test_scanner)
{
  ...
  for (int i = 0; i < 100; i++) {
    ...
    rid.page_num = 0;
    rid.slot_num = key;
    rc = handler->insert_entry((const char *)&key, &rid, false);
    ASSERT_EQ(RC::SUCCESS, rc);
  }
