...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) || is_blank(create_index.attribute_names[0].c_str())) {
  ...
  if (create_index.is_multi_index) {
  ...
  if (create_index.is_unique) {
    CreateIndexStmt* create_index_stmt = (CreateIndexStmt*)stmt;
    create_index_stmt->set_unique(true);
  }
  return RC::SUCCESS;
}
