...
struct UpdateField
{
  ...
  // 构造函数用于初始化普通值的字段
  UpdateField(const std::string &name, const Value &val) : field_name(name), value(val), is_subquery(false) {}

  // 构造函数用于初始化子查询值的字段
  UpdateField(const std::string &name, ParsedSqlNode *subq) : field_name(name), subquery(subq), is_subquery(true) {}
};
...
{
  Expression *                                       relation_name;  ///< Relation to update
